...
export const CONFIG_DEFINITIONS: Record<string, { label: string; description?: string }> = {
  ...
  wechat_sync_buf: { label: '微信同步缓冲', description: '消息同步游标（旧SDK，保留兼容）' },
  wechat_sdk_credentials: { label: '微信SDK凭证', description: '@wechatbot/wechatbot SDK 存储的登录凭证' },
  wechat_sdk_cursor: { label: '微信SDK游标', description: '@wechatbot/wechatbot SDK 长轮询游标' },
  ...
  hdhive_cookie: { label: '影巢Cookie', description: '影巢登录Cookie，需包含有效登录态' },
  hdhive_base_url: { label: '影巢站点地址', description: '影巢站点地址，留空时使用默认 https://hdhive.com' },
