...
function cleanQuery(text: string): string {
  ...
  cleaned = cleaned.replace(/\b(DTS[-\s.]?HD[-\s.]?MA|DTS[-\s.]?HD|DTS\d*(?:\.\d+)?|DDP\d*\.?\d*|DD\+?\d*\.?\d*|EAC3|E-AC-3|AC[-\s]?3|AAC\d*\.?\d*|TrueHD|Atmos|FLAC|LPCM|PCM|Dolby\s*Digital\s*Plus|Dolby\s*Atmos)\b/gi, ' ')
  ...
  cleaned = cleaned.replace(/\b\d{1,2}\b(?=\s*(?:-|@)?\s*(?:HiveWeb|RARBG|YTS|ETTV|$))/gi, ' ')
